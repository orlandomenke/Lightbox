        ...
        try
        {
            ...
            switch (ToolMode)
            {
                ...
                case CanvasToolMode.Move:
                    // If placements, guides, or reference boxes are selected, move them; otherwise move content.
                    e.Pointer.Capture(this);
                    if (_selectionManager?.HasSelection == true)
                    {
                        if (_selectionManager.SelectedPlacementIds.Count > 0)
                        {
                            _movingPlacements = true;
                            _placementMoveLast = (x, y);
                            PlacementsMoveStarted?.Invoke(0, 0);
                        }
                        else if (_selectionManager.SelectedGuideIndices.Count > 0)
                        {
                            _movingGuides = true;
                            _guideMoveLast = (x, y);
                            GuidesMovedStarted?.Invoke();
                        }
                        else if (_selectionManager.SelectedRefBoxIndices.Count > 0)
                        {
