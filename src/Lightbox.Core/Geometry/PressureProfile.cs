...
public sealed class PressureProfile
{
    ...
    public int SampleCount => _at.Length;

    /// <summary>The pressure at each sample, for tests and for reading a taper off.</summary>
    public IReadOnlyList<double> Samples => _pressure;

    /// <summary>
    /// A profile of one constant weight, for a path that was authored rather
    /// than drawn.
    /// </summary>
    /// <remarks>
    /// A pen line has no pressure to read — nothing was pressing — so the width
    /// tool has to start it from somewhere. Flat at full weight is what the line
    /// already renders as, which makes the first drag an edit rather than a jump.
    /// </remarks>
    public static PressureProfile Uniform(double pressure = 1.0, int samples = DefaultSamples)
    {
        ...
        {
            at[i] = (double)i / (count - 1);
            value[i] = pressure;
        }
        return new PressureProfile(at, value);
    }

    /// <summary>
    /// Samples a width edit needs to have somewhere to land.
    /// </summary>
    /// <remarks>
    /// <b>A drawn line has hundreds of samples and a straight authored segment
    /// has two</b>, and the width tool cannot put a local change into two. Sixty
    /// four is a sample every 1.5% of the line, which is finer than the reach of
    /// any single drag and cheap in a record that already holds hundreds of
    /// points per stroke.
    /// </remarks>
    public const int DefaultSamples = 64;

    /// <summary>
    /// The same curve, re-read at uniform positions along the line.
    /// </summary>
    /// <remarks>
    /// Lossy where the original was finer than the new spacing, which is why
    /// nothing calls it except the width tool — reshaping keeps every drawn
    /// sample. The width tool is editing the weight, so a resample it then
    /// overwrites is not a loss.
    /// </remarks>
    public PressureProfile Resampled(int samples)
    {
        var count = Math.Max(2, samples);
        if (count == _at.Length) return this;
    ...
    /// <summary>
    /// Push the weight up or down around one place on the line.
    /// </summary>
    /// <param name="at">Where, as a fraction of the line's length.</param>
    /// <param name="delta">How much, in pressure. Negative narrows.</param>
    /// <param name="reach">
    /// How far either side the change spreads, in the same fraction-of-the-line
    /// units. The caller derives it from a document distance, so a long line and
    /// a short one respond to the same drag the same way.
    /// </param>
    /// <remarks>
    /// <para>
    /// <b>A raised cosine, not a step or a triangle.</b> The falloff has to be
    /// smooth at both ends or the edit leaves a visible corner in the width where
    /// it stopped — a triangle is continuous but its slope is not, and a brush
    /// that changes taper abruptly reads as a nick in the line. Cosine is the
    /// cheapest window with a continuous derivative at both ends.
    /// </para>
    /// <para>
    /// Returns a new profile: the old one is what an undo restores, and reusing
    /// the array would leave the history pointing at the edited values.
    /// </para>
    /// </remarks>
    public PressureProfile Adjusted(double at, double delta, double reach)
    {
        if (delta == 0 || reach <= 0) return this;

        var value = new double[_pressure.Length];
        for (var i = 0; i < _pressure.Length; i++)
        {
            var u = Math.Abs(_at[i] - at) / reach;
            var weight = u >= 1 ? 0 : 0.5 * (1 + Math.Cos(Math.PI * u));
            value[i] = Math.Clamp(_pressure[i] + delta * weight, 0, 1);
        }
        return new PressureProfile([.. _at], value);
    }
