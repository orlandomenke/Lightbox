...
/// the rest of the same drag — and it was 29 and 32; the bar is set at 10, far
/// enough above the fixed build to survive a bad minute and far enough below
/// the broken one to catch it coming back.
/// </para>
/// <para>
...
{
    /// <summary>A page-sized document with sixty short marks scattered over it.</summary>
    private static MainViewModel Sketch(int strokes = 60)
    {
        var vm = new MainViewModel(null);
        vm.NewDocument(new NewDocumentSettings("probe", 1920, 1080, 12, 72, "#ffffff", true));
        ...
        {
            double x = 300 + (rnd.NextDouble() * 900), y = 200 + (rnd.NextDouble() * 600);
            vm.BeginStroke(x, y, 1);
            for (var k = 0; k < 6; k++)
            {
                x += (rnd.NextDouble() * 40) - 20;
                y += (rnd.NextDouble() * 40) - 20;
                vm.MoveStroke(x, y, 1);
            }
            vm.EndStroke();
        }
    ...
    private static void Marquee(MainViewModel vm) =>
        vm.ApplySelectionShape(
            [new(300, 200, 1), new(1200, 200, 1), new(1200, 800, 1), new(300, 800, 1)],
            false, false);
    ...
    private static (double First, double Later, double Commit) OneDrag()
    {
        ...
        var commit = Ms(() => vm.CommitTransformAffine(760, 500, 1, 1, 8 * Math.PI / 180, 0, 0));
