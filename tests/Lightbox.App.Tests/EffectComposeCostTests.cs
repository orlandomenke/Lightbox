...
public class EffectComposeCostTests(ITestOutputHelper output)
{
    ...
    public void AnAdjustmentPassCostsItsFilterAndNotAnOrderMore()
    {
        ...
        var sharpened = new List<RenderPass>
        {
            new(bottom, null, 1),
            new(null, null, 1, AdjustStack: Stack("detail.sharpen", ("amount", 100.0))),
        };
        ...
        var sharpen = MsPerCompose(sharpened);
        output.WriteLine(
            ...
            + $"+sharpen {sharpen:F2} ms ({sharpen / baseline:F1}x)");
        ...
        // A 3x3 convolution is a kernel like the blur, and a much smaller one.
        Assert.True(sharpen < baseline * 150 + 120,
            $"a 3x3 convolution should cost its kernel and nothing structural, got {sharpen:F2} ms over {baseline:F2} ms");
    }
