...
- [ ] **B54** `P3` `brush` The live blur still over-covers by ~88 px because the draft snapshot is cropped per segment `evidence: manual`
...
- [x] **B53** `P2` `project` CI runs the tests on a .NET 8 runtime it never asks for `evidence: CiRuntimeTests, TheJobThatRunsTestsAsksForTheRuntimeThoseTestsNeed, TheSdkIsStillNamedToo, TheEightPointZeroRuntimeIsStillActuallyRequired`
  ...
  - It also says when it is **spent**: move the solution to `net10.0` and `TheEightPointZeroRuntimeIsStillActuallyRequired` fails telling you the extra line and this test can both go. An expiring workaround that cannot announce its own expiry is how the original problem survived. Cost: S
...
- [x] **B31** `P2` `ai` Every AI call re-renders and re-encodes the reference views on the UI thread `evidence: ReferenceImagePayloadTests, ReferenceViewsAreEncodedOnceNotPerCall, TheEncodedViewIsReusedWhileTheSheetIsUnchanged, EditingTheDrawingThrowsTheEncodedViewAway, HidingALayerChangesWhatIsSent, TheLongEdgeIsCappedOnTheWayOutRatherThanOnTheView, ASmallViewIsNotUpscaledToTheCap, TheDownscaledReferenceStillHasTheDrawingInIt`
  ...
  - art-director's verdict was **accept-with-condition**: the cache costs nothing in expression, and the 768 px cap measurably loses facial construction on a face close-up and on a naturally-small head. Recorded as **Q21** with the four ways out rather than settled by whoever ran last — and with the caveat that the fixture's thin lines were drawn at pressure 0.25–0.5, where pressure drives flow as well as size, so the cap made a marginal line invisible rather than a solid line marginal.
