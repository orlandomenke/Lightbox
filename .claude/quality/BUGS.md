...
- [ ] **B37** `P1` `brush` Smudge, blender and blur smear black instead of what is on the canvas `evidence: manual`
  - Repro: draw the three medium strokes, then drag Smudge, Blender and Blur across them. All three lay down near-black, not the pale grey that is visibly there. One thin blur stroke came out as a blurred core inside a solid black surround, with nothing else touched.
  - Reported as "it picks the raw color, not the canvas representation".
  - **Not yet attributed, and this entry says so rather than guessing.** Two things were ruled out by reading: the committed path returns early for `BrushKind.Smudge`/`Blur` before the medium pass (`BrushEngine.cs:83-96`), so black pigment cannot arrive from a medium block carried over on the preset; and B33's compounding is a blur-spread bug, not a colour one.
  - What is still open, in the order worth checking: whether `ApplyPreset` actually carries `Kind`, because a smudge whose Kind stayed `Paint` falls through to `StampPaint` and deposits the foreground colour — which is black, and would look exactly like this; whether `SampledSource` returns something empty when the medium strokes are on another layer, leaving nothing to smear; and whether the live preview deposits paint that the commit then fails to replace, which is B4's old shape and would explain a black surround around a working blur core.
  - P1 because it makes three tools unusable rather than merely wrong-looking, and because the black is *deposited pigment* — it damages the drawing rather than mis-rendering it. Cost: unknown until attributed.
