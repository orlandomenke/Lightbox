...
- [ ] **B37** `P1` `brush` Smudge, blender and blur turn semi-transparent art opaque black `evidence: manual`
  - Repro: three medium strokes (Watercolor, Gouache, Oil) on a layer, then drag Smudge, Blender and Blur across them **on the same layer**. All three lay down near-black instead of the pale grey that is visibly there. One thin blur stroke came out as a blurred core inside a solid black surround.
  - **Attributed.** An effect dab composites **over** what the previous dab left instead of replacing it. `StampSmudgeDabs` builds `new SKPaint { IsAntialias = … }` — blend mode defaults to `SrcOver` — and then `target.Flush()` on purpose so the next sample sees the deposit (`BrushEngine.cs:1305-1318`). `StampBlur` has the identical shape: `DrawImage(snapshot, whole, sampling, blurPaint)` once per dab, no blend mode set.
  - Why that turns pale art black. The pale look of a watercolour stroke is **black pigment at low alpha over white paper**, not grey pixels: roughly `(0,0,0,60)`. The sample is therefore *correct* — and then it is deposited once per overlapping dab. At `Spacing = 0.1` about ten dabs cover any pixel, so alpha goes `1-(1-0.24·strength)^10` — **0.72 at strength 0.5** — and the colour underneath it all is pure black. The saturation trap from `CLAUDE.md`, in a new place: the sampled colour was right and the accumulation was not.
  - It also explains the surround precisely: blur draws a semi-transparent blurred snapshot over itself once per dab, so transparent canvas beside the stroke gains alpha from the blur's spread and then stacks it to opaque. "A blurred stroke surrounded by a full black artifact" is that, exactly.
  - **Why no test caught it:** every existing smudge and blur test works on an **opaque** black bar, where alpha is already 255 and `SrcOver` is indistinguishable from `Src`. The bug needs semi-transparent artwork to appear, and that is what every medium brush produces. Any regression test for this must smear something with alpha below 1.
  - Fix is a design change, not a blend-mode flip. `Src` would erase to transparent outside the dab's hard core, which is worse. What the deposit means is *lerp the canvas toward the sampled colour, weighted by the dab falloff*, and Skia has no single blend mode for that. The shape that already works in this engine is the paint path's: accumulate into a stroke-local scratch at full strength and composite once (`ComposeDraftRegion`). Effect brushes never got that treatment because they mutate the target directly. Measure before choosing. Cost: M
  - P1 because the black is deposited pigment — it damages the drawing rather than mis-rendering it — and it makes three tools unusable on exactly the artwork the medium brushes exist to make.
