...
- [~] Symmetry and mirrored painting `evidence: SymmetryAxis, SymmetryAxisTests, SymmetryStampingTests, AMirroredStrokeIsThePixelMirrorOfTheOneDrawn, AnIdentityAxisRendersByteForByteAsNoAxisAtAll, AStrokeDrawnWithoutSymmetrySerializesNoSymmetryKey, SymmetryLiveRegionTests, AReflectionIsOnScreenWhileTheStrokeIsStillBeingDrawn, ASymmetricStrokeCrossingItsOwnAxisPreviewsWhatItWillCommit, SymmetryAxisGizmo, TheAxisCanBeDraggedAndTurned`
  - Krita and Photoshop both have it, and for character design — the thing this application is for — a vertical mirror is not a nicety. The design question was whether a mirrored mark is *one stroke rendered twice* or *two strokes*, and **Q15 answered (c)**: one stroke while drawing, with an explicit "break symmetry" that expands it to two. So `Mirror` lives **on the stroke** rather than on the scene, which is the part that could not be deferred. **Q185 settled the rest on 2026-09-10** and the design is now whole: one record (`Symmetry { CenterX, CenterY, AngleDeg, Order, Mirror }`) covering mirror, radial and kaleidoscope in one shape; the copies made by **reflecting the canvas rather than the geometry**, which is invariant 7's argument generalised — `Hash01` then seeds every copy from the coordinates the artist drew, so a reflected mark is a true mirror instead of a re-rolled one; and the first landing reaching brush, eraser and smudge, which already share `StampStroke`. Fill and selections are excluded on purpose — a mirrored fill reflects contours, not dabs, and needs its own question. **The performance baseline landed first, deliberately** (`DrawingCostBaselineTests`, recorded at `ca00483d` before any symmetry code existed): a render fingerprint, cost-follows-the-segment at 1.00–1.01× for 4× the page area, linearity at 4.02–4.09× for 4× the marks, and a dirty region of 1312 px² against an analytic 1312. The thing it exists to catch is the union trap — N copies must publish N rectangles, never one enclosing them, which for a radial order is nearly the whole page. Unblocked.
  - **The record and the stamping landed 2026-09-10.** `SymmetryAxis` on the stroke, `Placements()` deciding the arrangement in Core without a canvas, and `StampStroke` looping over the placements on the paint path — one dab walk, N stamps, each computing its own bounded region. **The baseline it was built against did not move**: all three render fingerprints byte-identical, 1.00x for 4x the page area, dirty region 1312 px² against an analytic 1312. A mirrored mark is a true mirror — 25 of 2066 inked pixels disagree across the axis at a median of 1/255, against ~1033 for the same mark unmirrored, and turning the edge hard drops it to 8, which localises the residue to Skia's antialiasing of a negative-determinant matrix and then to pixel-centre tie-breaking. Neither is fixable from here and neither is visible.
  - `SymmetryPlacement` is deliberately **not** an evidence anchor even though it exists: it is a `readonly record struct`, and `codemap.py` indexes no type of that shape — 115 of them in `src/`, `StrokePoint` and `Dab` among them — so an anchor naming one can never resolve and would pin this item open for ever. `SymmetryAxis` and `SymmetryAxisTests` cover the record between them.
  - **The live preview landed the same day, and the design is the interesting part.** The tail machinery in the live scratch exists because *provisional* dabs move between pointer events, so their pixels are lent and taken back. A **settled** dab never moves again — so a copy of one is stamped permanently, with no backup, no rollback and no per-copy state at all, which is what leaves the hottest code in the application unchanged rather than merely equivalent. What it costs is lag: a copy trails the drawn mark by whatever has not settled, and the commit renders every copy exactly. **Measured rather than estimated, because the first estimate was wrong** — "a few pixels behind" was the claim; the reflected half of a mid-stroke mark holds 912 px against the drawn half's 1184, and in the region a copy owns outright 104 px of 427 are still missing from the preview. A visible quarter, not a few pixels. Closing it means giving each copy its own provisional tail — a backup and a rollback per copy per event, which is the cost the design exists to avoid, so this is a trade rather than an oversight. Both live routes are covered — the per-dab one and the silhouette one that the hard round family (Ink, hard round, the eraser) takes, which is the line art symmetry is actually used with. The copies are stamped *before* the drawn mark lends its tail, so a copy landing inside that tail is in the backup and survives the rollback; `ASymmetricStrokeCrossingItsOwnAxisPreviewsWhatItWillCommit` drives a diagonal across its own axis and finds **1 px of 427** inked in the preview where the commit has paper — extra ink is the smear, and there is none. Diagonal on purpose: a horizontal stroke's reflection lands on top of itself, so no region can tell a reflection that is present from one that is missing, and the first version of that test used one and could not fail. Both live tests were checked against the mutation that removes the copy stamping, and both go red.
  - **Granulation was broken on every copy, and adversary is what found it.** `ApplyGranulation` draws its noise mask over `rect` with `DstIn`, and it is called inside the block where the canvas carries the copy's transform — so `rect`, already this copy's rectangle, was transformed a second time, landed outside the copy's own scratch and was clipped away in silence. The mask carves alpha, so the symptom was the reflected copy coming out *more opaque* than the drawn one: measured at mean alpha 210.8 against 143.7, a 1.467x ratio, now 1.048x. **Every symmetry test set `Granulation` to 0**, which is why nothing caught it — the same blindness `brush-measurement` warns about, one setting along. Fixed by drawing the *authored* rectangle, which the active matrix maps onto the copy, and handing the shader the copy's inverse so the grain is sampled at the copy's own document position rather than reflected along with the mark — the paper is anchored to the document, and real paper is not mirrored.
  - **Two live-preview fidelity gaps remain, both found by adversary and neither reaching saved art.** For a silhouette brush the live path accumulates every copy into one shared max-coverage buffer while the commit stamps each copy independently and stacks them `SrcOver`, so where a mark crosses its own axis the overlap darkens the instant it commits. And `CapIsTheWholePass` computes its band from the drawn mark's dabs alone, so a copy's footprint ceiling is not applied live for the soft brushes that need one — the copy re-caps visibly at pen lift. Both are preview-versus-commit differences with the commit correct, and both want their own branch and their own fixture.
  - **One measured limitation, and it is in the publish rather than in symmetry.** `PublishState.MarkDirty` unions everything marked since the last publish into a single rectangle, so marking a region per copy repaints the box enclosing them rather than the copies. The cost is bounded by the **distance from the axis, not by the canvas**: the enclosing box is roughly `(2d + 2·reach) × (2·reach)` for a mirror at distance `d`, so an axis near the subject — which is what character design does — stays cheap, and it grows only as the artist works further from it. Measured at 6.4% of a 400×300 page against 0.21% for an un-mirrored event on its own baseline page. The fix is to carry disjoint regions through route selection, the compose ring and the tiled path, which is its own piece of work in `MainViewModel.Rendering.cs`; `PassSpec` already carries an `SKMatrix?`, so drawing the live overlay per copy at compose time is the likely shape of it.
  - **Still to come, and the reason this is not ticked:** the axis gizmo (place, drag, turn), the tool-options row, a shortcut in `ShortcutMap`, persisting the axis on the document beside rulers and guides, and the manual section — the axis is view-model state today, so nothing an artist can reach yet. Fill, smudge and blur are excluded on purpose: fill reflects contours rather than dabs, and the two effect brushes read pixels, which a canvas transform does not carry — see the coordinate-space note on `ToSurface`. Each needs its own pass and its own evidence.
...
7. **Upgrade**: "Symmetry and mirrored painting" [~]
   - Current status: **record, stamping and live preview landed 2026-09-10** — see the pillar entry. The dab
     re-seeding question this used to name is answered (Q185): the copies reflect the *canvas*, not the
     geometry, so `Hash01` seeds every copy from the coordinates the artist drew.
   - Market: Essential for character animation; every tool has it
   - Remaining: the axis gizmo and tool-options row, a `ShortcutMap` entry, persisting the axis on the
     document, the manual section — and separately, disjoint dirty regions in the publish path, which is
     what stops a radial order repainting the box enclosing its copies
   - Evidence: SymmetryAxis, SymmetryAxisTests, SymmetryStampingTests, SymmetryLiveRegionTests
