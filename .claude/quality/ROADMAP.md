...
- [ ] Hitbox and hurtbox editor — one canvas overlay, shared with the anchors `evidence: ShapeOverlay, ShapeOverlayTests, DraggingAHandleResizesOnlyTheFrameItWasDraggedOn`
  - **Not built, and not counted as built because the record is.** The declaration, the multi-frame set/clear, the resolution through holds and the whole export path are in and tested; what is missing is the tool that drags a rectangle into place on the canvas. An "editor" with no editing in it is exactly the green box `ROADMAP.md` exists to prevent, so this stays open and the manual lists it under Planned.
  - Deliberately **one** piece of work for anchors and shapes together rather than two. That is why the two records were built to the same shape: one overlay, one hit-test, one set of handles, two things it can place. Splitting it would mean building the overlay twice and then reconciling them.
  - What the record already settles, so the overlay does not have to decide it: **absence is the off state**. A sword connects on two frames of a six-frame swing, so the rectangle exists on those two drawings and nowhere else. There is no active flag for a re-time to desynchronise, and `AHitboxIsActiveOnlyWhereItIsPlaced` holds that line.
  - Resolution through holds is likewise already done, and it is not tidiness: a hurtbox that vanished on every other frame of a sequence on 2s would make the character invulnerable half the time — a gameplay bug with no visible cause in the drawing.
