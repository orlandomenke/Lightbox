...
- [ ] A guide set lands relative to the canvas it arrives on — authored on 4K, pulled into 1080p, the character is still the same height in frame (Q181) `evidence: GuideSetCanvas, AGuideSetRemembersTheCanvasItWasAuthoredOn, PullingA4kSetInto1080pKeepsTheHeightScaleTheSameFractionOfFrame, AGuideSetScalesUniformlySoALinesAngleAndAGridsSquarenessSurvive, ASetWithNoAuthoredCanvasPullsExactlyAsItDidBefore, ANewDocumentInAScopedFolderOpensWithItsGuides`
  ...
  - **And it applies on create** (Q181, decision 3): a folder that scopes a set hands its guides to a document created under it, scaled. "Heights stay the same throughout a project" is not a menu item you remember to use.
