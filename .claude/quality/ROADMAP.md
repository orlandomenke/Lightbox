...
- [~] Grade the model you brought, before you depend on it `evidence: GoldenSet, CapabilityProfile, CapabilityProfiler, FreeEngineArtist, GoldenSetTests, TheFreeEngineClearsEveryConstructedPair, TheLadderFindsWhereAModelStopsCoping, TheOrganicCategoryReportsItselfUnmeasuredRatherThanVanishing, TheArcRowTellsAnArcApartFromAChord, AiCapabilityPage`
  ...
  - **The half still open is the surface.** Nothing an artist can press yet: the profile is a record and a runner. The page that runs it, shows the cost before spending it, and stores the result per connection is the next branch, and `AiCapabilityPage` is the anchor that will resolve when it lands. Capturing hand-drawn `Organic` pairs from a document follows it.
