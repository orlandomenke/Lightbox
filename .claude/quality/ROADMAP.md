...
- [~] Symmetry and mirrored painting `evidence: SymmetryAxis, SymmetryAxisTests, SymmetryStampingTests, AMirroredStrokeIsThePixelMirrorOfTheOneDrawn, AnIdentityAxisRendersByteForByteAsNoAxisAtAll, AStrokeDrawnWithoutSymmetrySerializesNoSymmetryKey, SymmetryLiveRegionTests, AReflectionIsOnScreenWhileTheStrokeIsStillBeingDrawn, ASymmetricStrokeCrossingItsOwnAxisMatchesTheExactRender, SymmetryAxisGizmo, TheAxisCanBeDraggedAndTurned`
  ...
  - **The live preview landed the same day, and the design is the interesting part.** The tail machinery in the live scratch exists because *provisional* dabs move between pointer events, so their pixels are lent and taken back. A **settled** dab never moves again — so a copy of one is stamped permanently, with no backup, no rollback and no per-copy state at all, which is what leaves the hottest code in the application unchanged rather than merely equivalent. What it costs is one event of lag: a copy trails the drawn mark by the few dabs still under the pen, catching up continuously, and the commit renders every copy exactly. Both live routes are covered — the per-dab one and the silhouette one that the hard round family (Ink, hard round, the eraser) takes, which is the line art symmetry is actually used with. The copies are stamped *before* the drawn mark lends its tail, so a copy landing inside that tail is in the backup and survives the rollback; `ASymmetricStrokeCrossingItsOwnAxisMatchesTheExactRender` drives a stroke across its own axis and finds **0** mirrored pairs disagreeing.
