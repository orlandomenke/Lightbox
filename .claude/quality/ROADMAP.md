...
- [ ] Rig library — armatures saved, scoped and pulled, sized in head units off the document's height scale (Q181) `evidence: RigSet, RigScopes, RigSetEditor, PullRigCommand, ARigSetCanBeMadeAndNamed, ARigRemembersHowManyHeadsTallItWas, PullingARigAgainstAHeightScaleStandsItOnTheAnchorAtItsHeadCount, ARigWithNoHeadCountFallsBackToTheCanvasFraction, OriginalSizeLandsTheBindPoseUntouched, AProportionGhostIsNotTheDocumentsArmature, ManyGhostsCanStandOnOneDocument, RescalingABoundArmatureIsRefused`
  ...
  - **Blocked on** bones phase 1 (`docs/DESIGN-bones.md`) being scheduled: there is a rig record and a bone tool to save *from*, or there is nothing to put in a library.
