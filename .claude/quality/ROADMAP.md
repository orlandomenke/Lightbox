...
- [ ] Wet-into-wet across strokes — the cross-stroke wet window `docs/DESIGN-fluid-media.md` decided `evidence: WetStrokes, TwoOverlappingWetStrokesFuseIntoOneWash`
  ...
  - **Measured, because it is the larger half of why a wash looks wrong** (B279 is the other half). Laying overlapping watercolour strokes to fill an area leaves visible pale seams along every join: mottle at a 16 px box runs **6.3%** across the swatch against **3.4%** for one broad stroke covering the same ground, and setting `Rewetting` to 0 takes the swatch's figure to 4.2% — so a good part of the seam is the new stroke lifting and dragging its neighbour's pigment rather than joining it. A single wide stroke is already five times more even than the equivalent wash, which is the wrong way round: filling an area is the commonest thing a watercolourist does.
