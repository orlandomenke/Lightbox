...
- [ ] A saved rig can be placed as a proportion guide rather than as the armature (Q181, decision 2) `evidence: RigGhost, AProportionGhostIsNotTheDocumentsArmature, ManyGhostsCanStandOnOneDocument, AGhostIsNotHitByAPress, GhostsAreAbsentFromADocumentThatHasNone`
  - **Cost M, and `Doc.Armature` being singular is what forces it.** Human, dog and goblin standing together on one sheet is the use the library was asked for, and it is not expressible as an armature — a document has one. The ghost is drawn and snapped like a guide, is not posable, and any number stand on one drawing.
  - The record, the overlay and the hit-transparency are the work; the sizing is already done, because a ghost lands through the same `ArmatureFit` the armature landing uses.
