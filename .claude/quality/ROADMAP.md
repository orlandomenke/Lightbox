...
- [ ] The wet edge comes back without hollowing the stroke — capillary pull conserves an interior floor `evidence: AWetEdgeRimLeavesTheMiddleStanding, AWetStrokeDoesNotDryWithAPaleSpine`
  - B274/B275. The dark dried rim is the mark watercolour is named for, and it is shipped at `EdgePull 0.06` — effectively off — because the pull drains the interior one-for-one (B35). The fix is a floor inside `CapillaryPull`/`Deposit`: the rim may only take pigment above a fraction of what the wash deposited. Needs its own measurement pass across a stroke's profile before and after.
- [ ] The fluid solver has no favourite direction — isotropic transport, and a symmetric mark that dries symmetric `evidence: ASymmetricMarkDriesSymmetric, ADiscDriesIsotropic`
  - B276. Measured on a disc seeded symmetric by construction, the shipped solver dries it **32.7% heavier on one side** and leaves the axes 30.2% out of true against the diagonals — the first is the one-sided toothed fringe on a wet stroke, the second is the cross through a blot. Three orderings were built and measured and none is a clean win: alternating sweeps barely move it, damped Jacobi zeroes the lopsidedness but worsens the cross and costs 2.5×, and red-black is best on every quality metric and stops a lone wet cell blooming altogether. The lopsidedness is sweep order; the cross is the 4-neighbour stencil, which reaches a diagonal only in two steps. So the real work is diagonal fluxes in the transport step with blooming preserved and convergence pinned — the entry carries the numbers and the calibration lever.
- [x] Wet-into-wet across strokes — the cross-stroke wet window `docs/DESIGN-fluid-media.md` decided `evidence: WetStrokes, WetRun, WetRunCommit, AFusedWashIsFlatterThanTheSameStrokesDriedOneByOne, PaintingAWashStrokeByStrokeMatchesReloadingIt, AWashLooksTheSameLiveAsCommitted, AWashIsFlatterWithTheWindowOnThanWithout`
  - Today every stroke rents a cleared lattice and dries unconditionally, so parallel washes stay three ribbons with seams instead of fusing — `Rewetting` copies colour into the new stroke but never re-opens the old one. The design doc records the wet window as decided and sequences it; this line exists so the roadmap owns it rather than only the doc.
  - **Measured, because it is the larger half of why a wash looks wrong** (B279 is the other half). Laying overlapping watercolour strokes to fill an area leaves visible pale seams along every join: mottle at a 16 px box runs **6.3%** across the swatch against **3.4%** for one broad stroke covering the same ground. Filling an area is the commonest thing a watercolourist does, and it is the least even thing the medium produces.
  - **Spiked 2026-08-22 and worth 5×**, which is what justifies the expensive half — `docs/DESIGN-fluid-media.md` → *The spike* carries the table. The same dab coverage laid as six strokes mottles at 11.1% (2 px) and as one stroke at **2.2%**, so the seam is neither the paper nor double-counted pigment: it is one wet region per stroke, and a wash is not a stack of those.
  - **The cheap alternative is measured and rejected.** Lift-and-erase — each stroke takes still-wet paint off the layer and re-deposits the combination, no snapshots — reaches 9.3%, a sixth of the gain, because the seam is *geometric*: overlapping soft-edged humps do not add to a flat field however honestly the pigment moves between them. It also costs preview fidelity, since lifting is a commit-path change and the preview is an overlay over the untouched layer: live-to-committed divergence went from 1/255 to 32/255, now guarded by `AMediumStrokeCrossingWetPaintLooksTheSameLiveAsCommitted`.
  - The integration to cost is the live path, not the simulation. Blur and smudge already paint into a copy of the layer rather than an overlay because they read it back; a medium that shares or moves paint belongs in that class, and putting it there is what makes preview and commit agree by construction.
  - **Landed 2026-08-22, and the live path cost less than that paragraph expected.** `BrushSettings.WetStrokes` says how many following strokes a paint stays wet for — nullable, so an unused brush writes no key — and `WetRun.Split` groups the record into the runs that must be simulated together. A run's dabs go into one scratch and the medium runs **once** over all of it, which is the whole fix: what made a wash ribbon was one wet region, one boundary and one drying *per stroke*. Through the app, six bands take row-to-row unevenness from **16.2% to 1.8%**.
  - **The interactive path did not need the copy-based class after all.** The commit keeps a copy of what was under the open run (`WetRunCommit`) and re-simulates the run rather than re-rendering the frame, so the work stays proportional to the run — the ceiling is six strokes over what they reach. The preview lifts that same run off the layer and draws the whole wash in the existing overlay, which is what makes them agree: live-to-committed divergence is **0.00/255 mean, 1 peak** with the window on, against 4.2/255 and **123** at worst without the lift. Losing the copy mid-wash — an eviction, an undo — falls back to a frame re-render rather than a seam.
  - **What it costs, recorded because it is visible.** A shared scratch saturates at one coat, so colour stops building up inside a window: six pale bands sit at 102/255 fused against 153 dried one at a time. That is what painting into standing water does, and it is why the presets use small windows (Watercolor 3, Ink wash 2) rather than the ceiling — the technique of deepening a value by going over it again has to still work above the window.
  - Still open: strokes from *different* brushes do not fuse, even inside a window. That is wet-into-wet between two paints and it needs the lattice seeded per stroke rather than once for the run; the wash case is the one an artist hits first, and it is the one this covers.
...
- [x] Three more footprints — drop, crescent, blot — and an alpha gradient every shape can wear `evidence: TipFalloff, ADropIsACircleAtZeroAndPointedAtOne, ACrescentBitesDeeperAsSharpnessRises, ABlotIsLobedRatherThanRound, FadePullsTheGradientInFromTheBoundary, AFadedChiselFadesEvenlyAroundItsBoundaryNotIntoABone, TheFourFalloffCurvesBakeFourDifferentEdges, TheSoftCircleFollowsTheChosenFalloffToo, AFadedRecipeSurvivesTheRoundTripThroughTheWindow`
  - The drop is the exact uneven-capsule distance (a disc at zero point, so the slider starts somewhere known); the blot wobbles a disc through hashed adjacent harmonics so a wash's edge stops reading as geometry. `Fade` is normalised per shape by its inradius — 1 always means "fades from the core", even on a chisel whose thickness is a fraction of the tip — and four falloff curves (smooth, linear, airbrush, dome) apply to any band, the soft circle's hardness included. Fade 0 re-bakes every old recipe bit-identically; the gradient distance switches on only with the band, so the crisp edge keeps its exact historical pixels.
