...
- [~] Infinite canvas `evidence: TileStore, TileGrid, StrokeIndex, TileCompositor, TileStoreTests, TileCullingTests, AnUntouchedTileIsNeverAllocated, RecompositingCostsWhatIsOnScreenNotWhatExists, PanningAcrossEmptySpaceAllocatesNothing, AFeatureLeftAtItsDefaultWritesNoKey, ATiledRenderIsBitIdenticalToAnUntiledOne, AMarkPaintedBeyondTheDocumentIsStored`
  ...
  - **The three steps that were "remaining, in order" have all landed** (2026-08-08, reconciled after the fact — each shipped under a different name than this line promised, which is why the plan read as open while the tree said otherwise). (1) **B82** — `SetViewport` reports the visible rectangle, `ComposeViewportCulled` composes it, and the snapshot carries the rectangle its image covers; the two scars it left, B120 and B121, are recorded on the bug. (2) The unbounded document property is `FeatureKey.UnboundedCanvas` under **Q21**'s answer exactly — a document property resolved through project defaults (`AProjectTypeSetsDefaultsRatherThanAvailability`), absent from the file at its default (`AFeatureLeftAtItsDefaultWritesNoKey`). (3) `FeatureConflict` declares `UnboundedCanvasExcludesFixedFrameBoundsExport`, and the sprite exporter names that reason instead of failing silently.
  - **What actually remains is the paintable extent: ink cannot land outside the document.** `StampPaint` sizes its scratch with the clamped `SegmentBounds`, so a stroke drawn off-canvas is pickable, movable and deletable (**B134**) but paints nothing — `AMarkPaintedBeyondTheDocumentIsStored` is the test that does not exist yet, and it is surgery on the hottest path in the repository, which is why B134's fix deliberately did not attempt it.
...
- [ ] Resize canvas and resize image `evidence: CanvasResize, ImageResize, ResizeAnchor, IPixelResampler, CanvasResizeTests, ImageResizeTests, NotOneCoordinateInTheDocumentMoves, GrowingToTheLeftMovesTheOriginRatherThanTheDrawing, GrowingAndCroppingByTheSameOddAmountReturnsToWhereItStarted, ADocumentThatNeverResizedWritesNoOriginKeys, DoublingTheImageDoublesTheGeometryAndTheBrush, EveryCoordinateOnTheSceneIsAccountedFor, ResizeDialog, ResizeDialogTests`
  ...
  - **What remains is the origin's cost, and it lands in the hottest file in the repository.** The document rectangle is no longer `(0, 0, W, H)`, so every conversion from a document coordinate to a pixel in a layer bitmap has to subtract the origin: `BrushEngine`'s clamps (`SegmentBounds`, `RangeBounds`), its two full-canvas rects, and the eight places that read `targetPixels` — alpha lock, the medium simulator's two reads, the smudge sample, the blur's subset and snapshot, and `BakeSample`. `InDocumentSpace` already translates by an arbitrary device origin, which is what makes this surgery rather than a rewrite. The guard worth having is one property test rather than eight unit tests: render a document at origin zero, render the same strokes in a document grown leftward, and assert the overlapping region is bit-identical — that fails for any of the sites got wrong, which is the point.
