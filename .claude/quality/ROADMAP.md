...
- [x] Timing presets — save an exposure pattern and apply it to a range of cels `evidence: TimingPreset, ApplyTiming, TimingPresetTests, ApplyingAPatternReExposesTheDrawingsThatAreThere, APatternShorterThanTheRangeRepeats, ItNeverCreatesOrDestroysADrawing`
  ...
  - **The engine has landed; the UI has not.** `ExposureSheet.ApplyTiming` re-times a range and `TimingPreset.BuiltIns` carries the six patterns worth having on day one. What is left is reaching it: a picker on the timeline's range selection, one undo step, and saving an artist's own patterns. Deliberately split, because the operation is the part with the semantics in it and it is testable without a window.
