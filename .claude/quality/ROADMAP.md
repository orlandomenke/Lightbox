...
- [ ] Godot exporter — a `.tres` resource, and the only one of the three that is plain text `evidence: GodotExporter, GodotResource, GodotExportTests, ASpriteFramesResourceListsEveryFrameInOrder, ATresFileIsWrittenBesideTheSheetAndNeverInsideProjectGodot`
  - Real work, unlike the two above: Godot has `AtlasTexture` and `SpriteFrames` resources, and an importer that produced them saves an artist building an animation frame by frame in the inspector.
  - **The one engine whose asset format we can legitimately write**, because `.tres` is a documented plain-text format. That is the whole reason it is next among the three rather than Unreal.
  - Still unverifiable here, so it inherits the rule below: record the format version it was written against, and get one real import before the box goes green. And it must write beside the sheet — never touch `project.godot` or the `.godot/` cache, which Godot owns the way Unity owns `.meta`.
...
- [?] Unreal Paper2D exporter — blocked by the file format, not by effort
  - **`.uasset` is a binary serialized format and cannot be written from outside the editor.** This is not a difficulty to push through; it is the reason the Unity approach does not transfer. Unity's importer works because Lightbox writes ordinary files and a C# editor script does the engine-side work — the equivalent here is an in-editor Python script or a commandlet, and *that* is the item, not a file writer.
  - Left `[?]` on purpose: it needs a decision about whether shipping an Unreal-side Python script is in scope at all, and guessing that is how you end up with a plugin nobody asked for. Recorded here so the next person does not spend a day discovering the binary format.
