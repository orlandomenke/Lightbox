# Q126 · What a crop is, and whether it touches the marks outside it — **answered 2026-08-19: paper only, menu commands only**
...
| | What it costs |
| --- | --- |
| **Menu commands only** (recommended, **chosen**) | Crop to selection and Trim to drawing, on the Image menu. Reuses the marquee contours and `BrushEngine.ReachBounds`; one `Perform` each. No canvas overlay, no new `ToolId`. |
| Menu commands **and** an interactive tool | A rubber-band with eight handles, Enter to apply, Escape to cancel — roughly triple the work: a `ToolId`, a canvas overlay, hit-testing, a tool-options bar and its own suite. |
...
**What the chosen option costs.** There is no drag-a-rectangle crop, so a crop
that is not already a selection means drawing a marquee first. That is one extra
gesture, and it is the gesture the marquee exists for — but it does mean the
common "just take a bit off the top" is two steps rather than one. The
interactive tool remains buildable on top of this without rework: both commands
funnel through `CanvasResize.CropTo`, which takes a rectangle and does not care
who chose it.
