...
- [ ] **B194** — run `get-build.ps1` with **`powershell`**, not `pwsh`. Windows PowerShell 5.1 is the shell the script is written for and the only one that reads a BOM-less file in the machine's ANSI code page, so it is the only one that can see a non-ASCII character in a string turn into a quote. `pwsh` reads the same file as UTF-8 and runs it happily — which is why a green suite and a working `pwsh` run together prove nothing here. (`WindowsHelperScriptTests` keeps the scripts ASCII so this cannot recur; this is the check that the rule is aimed at the right shell.)
- [ ] `get-build.ps1` creates the `Lightbox` junction beside the extracted build, **without admin rights and without Developer Mode** — that is the property that makes it a junction rather than a symlink, and nothing on a Linux runner can check it. `dir` shows it as `<JUNCTION>`; the printed *Claude Desktop command* path opens the server.
- [ ] Run `get-build.ps1` again with a **second** zip. The junction points at the new build and the **first build's folder still has its files** — the link is deleted, never what it points at. (The suite covers the delete-link-not-target logic with a symlink; that it holds for a real junction is here.)
- [ ] Put an ordinary folder called `Lightbox` beside the builds and run it again: the script **refuses and leaves the folder alone**, warning instead of deleting. A build extracted before junctions existed is that folder, so a silent overwrite would eat it.
...
- [ ] **Arrow, moving** — click a line and drag it. The **cyan trace follows the pointer while the drawing stays put**, and the drawing jumps to the trace on release. That is deliberate rather than a glitch: re-rendering the frame from its strokes on every pointer move is what invariant 6 forbids. What to check is that the two agree — the line lands exactly where the outline was.
- [ ] **Arrow, moving** — the moved line's **texture is different** where it landed, on a grainy brush. Q26 chose this; the manual says so. What would be a bug is the line changing *shape*, or the ends not matching the outline.
- [ ] **Arrow, moving** — press on a selected line and release without moving. Nothing is committed: undo does not step over a phantom move.
- [ ] **Arrow, moving** — drag a line, then drag off the window so capture is lost mid-drag. The outline snaps home and the drawing never moved.
- [ ] **Arrow, nudging** — select a line and press the arrow keys: it moves a pixel a press, ten with Shift. Then press ← with **nothing selected**: the frame steps. The keys are shared on purpose, and the condition is "Arrow tool *and* a selection".
- [ ] **Arrow, deleting** — select two lines and press Delete. Both go, and one undo brings both back. Check the **order**: a line that was behind another must come back behind it, not in front.
- [ ] **Arrow, recolouring** — select a line, choose a foreground colour, press **Recolour** in the tool options. The line takes it. Now do the same to a line **painted from a palette swatch**, then edit that swatch: the recoloured line no longer follows it, and the ones you did not touch still do. Undo puts the link back.
- [ ] **Arrow, locked layer** — lock the layer, then try Delete, a nudge and Recolour. Each says the layer is locked rather than doing nothing quietly.
